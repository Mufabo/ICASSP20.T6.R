...
# Using concrete values instead of variables due to scoping errors in EM.
# Partial doesn't circumvent this errors either
g <- list(gaus = function(t) g_gaus(t, 2),
        t = function(t) g_t(t, 2, 3),
      huber = function(t) g_huber(t, 2, list(1.794123, 0.8, 0.3802402)))

rho <- list(gaus = function(t) rho_gaus(t, 2),
        t = function(t) rho_t(t, 2, 3),
        huber = function(t) rho_huber(t, 2, list(1.794123, 0.8, 0.3802402)),
        tukey = function(t) rho_tukey(t, 2, 4.685)
        )

psi <- list(gaus = function(t) psi_gaus(t),
        t = function(t) psi_t(t, 2, 3),
        huber = function(t) psi_huber(t, 2, list(1.794123, 0.8)),
        tukey = function(t) psi_tukey(t, 4.685)
        )

eta <- list(gaus = function(t) eta_gaus(t),
        t = function(t) eta_t(t, 2, 3),
        huber = function(t) eta_huber(t, 2, list(1.794123, 0.8)),
        tukey = function(t) eta_tukey(t, 4.685)
        )
...
for(i_eps in 1:eps_iter){
  ...
  tmp <- foreach(iMC = 1:MC, .packages = c("zeallot", "ICASSP20.T6.R")) %dopar% {
  ...
  for(i_mc in 1:MC){
    bic_final[i_mc, i_eps,,,] <- tmp[[i_mc]]$bic
    pen_final[i_mc, i_eps,,,] <- tmp[[i_mc]]$pen
    like_final[i_mc, i_eps,,,] <- tmp[[i_mc]]$like
 } 
}
...
for(iEmBic in 1: embic_iter){
  for(k_bic in 1:dim(bic_final)[4]){
    ...
    par(mar=c(.5,1,5,2.5))
    image(z=t(seq(0,1,.1)), col=grDevices::rainbow(12), axes=FALSE, main="Slope", cex.main=.8)
